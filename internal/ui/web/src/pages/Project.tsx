...
export function ProjectPage() {
  ...
  async function run(label: string, fn: () => Promise<ToolkitResult>) {
    ...
    try {
      const r = await fn();
      setOutput(r.output || r.hint || (r.path ? `Wrote ${r.path}` : r.ok ? "OK" : r.error || "Done"));
      setCli(r.cli || "");
  ...
  return (
    <div className="stack">
      <div className="page-head">
        <h1>Project</h1>
        <p className="lead">Init scaffold, env profiles, backup/restore, and optional Kubernetes helpers.</p>
      </div>
      ...
      <section className="card panel">
          ...
          {profiles.map((p) => (
            <div className="url-row" key={p.name}>
              ...
              <button
                ...
                onClick={() => void run("use", () => toolkitJSON("/api/v1/env/use", { name: p.name })).then(() => refreshEnv())}
              >
                Use
              </button>
              {p.name !== "lab" && (
                <button
                  ...
                  disabled={!!busy}
                  onClick={() =>
                    void run("rm", () => toolkitJSON(`/api/v1/env/${encodeURIComponent(p.name)}`, undefined, "DELETE")).then(
                      () => refreshEnv(),
                    )
                  }
                >
        ...
        <div className="stack-sm" style={{ marginTop: "1rem" }}>
          ...
          <label className="field">
            ...
            <input value={orch} onChange={(e) => setOrch(e.target.value)} placeholder="https://…/v2" />
          </label>
          <button
            type="button"
      ...
      <section className="card panel">
        ...
        <p className="hint">Optional — needs kubectl + Camunda Helm. Compose-only labs can skip this.</p>
        <div className="panel-actions">
          <button type="button" disabled={!!busy} onClick={() => void run("k8s", () => getK8sStatus())}>
        ...
        <label className="field">
          ...
          <input type="number" min={0} value={replicas} onChange={(e) => setReplicas(Number(e.target.value))} />
        </label>
        <div className="panel-actions">
          <button
            type="button"
          ...
          <button
            ...
            onClick={() =>
              void run("k8s-scale", () =>
                toolkitJSON("/api/v1/k8s/scale", { component: k8sComponent, replicas, confirm: true }),
              )
            }
          >
            Scale
