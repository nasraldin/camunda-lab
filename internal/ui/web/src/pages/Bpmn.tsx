  ...
  return (
    <div className="stack">
      <div className="page-head">
        <h1>BPMN toolkit</h1>
        <p className="lead">Lint, diff, explain, review, generate tests, and scan — same engines as the CLI.</p>
      </div>
        ...
        {tab === "scan" ? (
          ...
          <div className="stack-sm">
            <label className="field">
              ...
              <input type="file" accept=".bpmn,application/xml,text/xml" onChange={(e) => setFile(e.target.files?.[0] || null)} />
            </label>
            <label className="field">
              <span>To (upload)</span>
              <input type="file" accept=".bpmn,application/xml,text/xml" onChange={(e) => setFileB(e.target.files?.[0] || null)} />
            </label>
            <p className="hint">Or absolute paths:</p>
            <label className="field">
              <span>From path</span>
              <input value={path} onChange={(e) => setPath(e.target.value)} placeholder="/Users/…/order-v1.bpmn" />
            </label>
            <label className="field">
              <span>To path</span>
              <input value={pathB} onChange={(e) => setPathB(e.target.value)} placeholder="/Users/…/order-v2.bpmn" />
            </label>
          </div>
        ) : (
          <div className="stack-sm">
            ...
            <label className="field">
              ...
              <input value={path} onChange={(e) => setPath(e.target.value)} placeholder="/Users/…/process.bpmn" />
            </label>
            {tab === "testgen" && (
              <label className="field">
                <span>Language</span>
      ...
      {result && (
        <section className="card panel">
          ...
          {result.cli && <p className="hint">CLI: <code>{result.cli}</code></p>}
          <pre className="code">{result.output || result.markdown || JSON.stringify(result, null, 2)}</pre>
        </section>
      )}
    </div>
  );
