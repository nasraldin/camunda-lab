...
export function LogsPage() {
  ...
  function start(follow: boolean) {
    ...
    const es = new EventSource(
      `/api/v1/logs/${encodeURIComponent(service)}?follow=${follow ? '1' : '0'}`,
    )
    esRef.current = es
    es.onmessage = (ev) => {
      setLines((prev) => {
    ...
    <div className="stack">
      <div className="page-head page-head-row">
        <div>
          <h1>Logs</h1>
          <p className="lead">
            Watch messages from one lab service. Search to filter or highlight lines when something
            fails.
          </p>
        </div>
